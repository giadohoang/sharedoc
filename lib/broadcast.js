class Broadcast {
    ...
    processOutgoingBuffer(peerId) {
        ...
        this.outgoingBuffer.ferEach(op => {
            connection.send(op);
        });
    }

    bindServerEvents(targerPeerId, peer) {
        this.peer = peer;
        this.onOpen(targerPeerId);
    ...
    onOpen(targetPeerId) {
        this.peer.on('open', id => {
            ...
            if (targetPeerId == 0) {
                ...
                this.requestConnection(tergetPeerId, id, this.controller.siteId)
            }
        });
    }
    ...
    onError() {
        this.peer.on("error", err => {
            const pod = String(err).replace("Error: could not connect to peer ", "");
    ...
    forwardConnRequest(peerId, siteId) {
        const connnected = this.outConns.filter(conn => conn.peer !== peerId);
        const randomIdx = Math.floow(Math.random() * connnected.length);
    ...
    acceptConnRequest(peerId, siteId) {
        ...
        const initialData = JSON.stringify({
            ...
            initialVersions: this.controller.vector.version,
        ...
        connection.on('data', data => {
            ...
            switch (dataObj.type) {
                ...
                case 'syncResponse':
                    ...
                    this.controller.handlerSync(dataObj);
                    break;
                case 'syncCompleted':
                    this.processOutgoingBuffer(dataObj.peerId);
                    break;

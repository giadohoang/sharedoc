...
function remoteInsertRandom(crdt, numberOfOperations) {
    ...
    return remoteInsertRandom(crdt, chars);
}
...
function avgIdLength(crdt) {
    let numChars = 0;
    const idArray = crdt.struct.map(line => line.map(char => char.position.map(
        id => id.digit)
        .join('')));
...
function addRowWithId(operations, crdt, func) {
    ...
    return `|${cell1}|${cell2}|${cell3}|${cell4}|${cell5}|${'-'.repeat(95)}`
}
...
function addRow(operations, crdt, func) {
    ...
    return `|${cell1}|${cell2}|${cell3}|${'-'.repeat(62)}`
}
